...
const useRegistrationForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (!validateUsernameLength(name)) {
      setNameError(FailedMessages.NameLength);
      error = true;
    }

    if (!error) {
      setLoading(true);
      const submit = await authService.signup({
        username: name,
        email,
        password,
      });
  ...
  const handleUsernameChange = (e: ChangeEvent<HTMLInputElement>) => {
    const normalizedString = normalizeSpaces(e.target.value);

    setName(normalizedString);
  };
